...
def conflict(index_np,dict_cu,w=16 * 10 ** 4,d=3,max_conf=1):
...
if __name__ == '__main__':
    ...
    cu_test_path = "../traindata_set_5s/traindata_160000_cu_5s/" + str(file_now).zfill(5) + ".txt"
    cu_sketch_now = np.loadtxt(cu_test_path)
    sketch_now = cu_sketch(cu_d=3, cu_w=cu_w, flag=1, dict_cu=dict_cu, cu_sketch_load=cu_sketch_now)


    cu_train_path = "../traindata_set_5s/traindata_160000_cu_5s/" + str(file_now1).zfill(5) + ".txt"
    cu_sketch_now1 = np.loadtxt(cu_train_path)
    sketch_now1 = cu_sketch(cu_d=3, cu_w=cu_w, flag=1, dict_cu=dict_cu, cu_sketch_load=cu_sketch_now1)
    ...
    dataset_x_ids = conflict(index_np=index_array,dict_cu=dict_cu,w=cu_w,max_conf=max_conf)

    # 获取test时刻流信息
    test_x_key = list(test_flows_data.keys())
    test_index_array = get_value(np.array(test_x_key))
    test_x_ids = conflict(index_np=test_index_array, dict_cu=dict_cu,w=cu_w,max_conf=max_conf)

    '''获取所在时刻的d个查询值'''
    # 获取train
    dataset_x = np.full((dataset_x_ids.shape[0], cu_d*(1+dataset_x_ids.shape[1])), 0)
    for i in range(cu_d*max_conf):
        conf_flows_id=np.where(dataset_x_ids[:,i]!=-1)[0]
        dataset_x[conf_flows_id,i*cu_d:(i+1)*cu_d]=sketch_now1.query_d_np(dataset_x_ids[conf_flows_id,i]).T
    dataset_x[:,cu_d*dataset_x_ids.shape[1]:]=sketch_now1.query_d_np(index_array).T

    # 获取test
    test_x = np.full((test_x_ids.shape[0], cu_d * (1+test_x_ids.shape[1])), 0)
    for i in range(cu_d * max_conf):
        conf_flows_id = np.where(test_x_ids[:, i] != -1)[0]
        test_x[conf_flows_id, i * cu_d:(i + 1) * cu_d] = sketch_now.query_d_np(test_x_ids[conf_flows_id, i]).T
    test_x[:, cu_d * test_x_ids.shape[1]:] = sketch_now.query_d_np(test_index_array).T

    # 查询所有流在cu sketch中的值
    cu_flows_query_d = sketch_now.query_d_np(test_index_array)
    cu_x = cu_flows_query_d.T
    # 流的查询值，即对每行求min:(n,1)
    flows_query_cu = np.min(cu_x, axis=1)
    cu_y = np.array(list(test_flows_data.values())).T
                ...
                for drop_out in drop_out_list:
                    for set_epoch in epoch_list:
                        for set_batch in batch_list:
                            ...
                            torch.save(dnn1net.state_dict(), "cu_dnn_d_params_"+str(max_conf)+".pkl")
