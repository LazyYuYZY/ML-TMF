...
def conflict(index_np,dict_count,w=16 * 10 ** 4,d=3,max_conf=1):
...
if __name__ == '__main__':
    ...
    while file_now1 == file_now:
        file_now1=random.randint(0,180-1)
    ...
    count_sketch_now = np.loadtxt(count_test_path)
    sketch_now = count_sketch(count_d=3, count_w=count_w, flag=1, dict_count=dict_count, count_sketch_load=count_sketch_now)


    count_train_path = "../traindata_set_5s/traindata_160000_count_5s/" + str(file_now1).zfill(5) + ".txt"
    count_sketch_now1 = np.loadtxt(count_train_path)
    sketch_now1 = count_sketch(count_d=3, count_w=count_w, flag=1, dict_count=dict_count, count_sketch_load=count_sketch_now1)
    ...
    dataset_x_ids = conflict(index_np=index_array,dict_count=dict_count,w=count_w,max_conf=max_conf)

    # 获取test时刻流信息
    test_x_key = list(test_flows_data.keys())
    test_index_array = get_value(np.array(test_x_key))
    test_x_ids = conflict(index_np=test_index_array, dict_count=dict_count,w=count_w,max_conf=max_conf)

    '''获取所在时刻的d个查询值'''
    # 获取train
    dataset_x = np.full((dataset_x_ids.shape[0], count_d*(1+dataset_x_ids.shape[1])), 0)
    for i in range(count_d*max_conf):
        conf_flows_id=np.where(dataset_x_ids[:,i]!=-1)[0]
        dataset_x[conf_flows_id,i*count_d:(i+1)*count_d]=sketch_now1.query_d_np(dataset_x_ids[conf_flows_id,i]).T
    dataset_x[:,count_d*dataset_x_ids.shape[1]:]=sketch_now1.query_d_np(index_array).T

    # 获取test
    test_x = np.full((test_x_ids.shape[0], count_d * (1+test_x_ids.shape[1])), 0)
    for i in range(count_d * max_conf):
        conf_flows_id = np.where(test_x_ids[:, i] != -1)[0]
        test_x[conf_flows_id, i * count_d:(i + 1) * count_d] = sketch_now.query_d_np(test_x_ids[conf_flows_id, i]).T
    test_x[:, count_d * test_x_ids.shape[1]:] = sketch_now.query_d_np(test_index_array).T

    # 查询所有流在cout sketch中的值
    cout_flows_query_d = sketch_now.query_d_np(test_index_array)
    cout_x = cout_flows_query_d.T
    cout_x[test_index_array % 2 != 0, :] = -cout_x[test_index_array % 2 != 0, :]
    # 流的查询值，即对每行求min:(n,1)
    flows_query_cout = np.median(cout_x, axis=1)
    cout_y = np.array(list(test_flows_data.values())).T

    # #获取cout的相对误差
    cout_num_array = flows_query_cout.T
    real_num_array = np.array(test_flows_data_list).reshape(1, -1)
    relative_cout_error = np.abs((real_num_array - cout_num_array) / real_num_array)
    print("relative error:" + "{:.6f}".format(np.mean(relative_cout_error)))

    '''train dataset'''
    dataset_x[dataset_x >= 0]=np.log(1+dataset_x[dataset_x >= 0])
    dataset_x[dataset_x < 0] = -np.log(1 - dataset_x[dataset_x < 0])
    dataset_y = np.log(np.array(train_flows_data_list).reshape(-1, 1))
                ...
                for drop_out in drop_out_list:
                    for set_epoch in epoch_list:
                        for set_batch in batch_list:
                            ...
                            torch.save(dnn1net.state_dict(), "count_dnn_d_params_"+str(max_conf)+".pkl")
