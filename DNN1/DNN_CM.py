...
if __name__ == '__main__':
    ...
    cm_test_path = "../traindata_set_5s/traindata_160000_cm_5s/" + str(file_now).zfill(5) + ".txt"
    cm_sketch_now = np.loadtxt(cm_test_path)
    sketch_now = cm_sketch(cm_d=3, cm_w=cm_w, flag=1, dict_cm=dict_cm, cm_sketch_load=cm_sketch_now)


    cm_train_path = "../traindata_set_5s/traindata_160000_cm_5s/" + str(file_now1).zfill(5) + ".txt"
    ...
    dataset_x_ids = conflict(index_np=index_array,dict_cm=dict_cm,w=cm_w,max_conf=max_conf)
    ...
    dataset_x[:,cm_d*dataset_x_ids.shape[1]:]=sketch_now1.query_d_np(index_array).T
    ...
    # 查询所有流在cm sketch中的值
    cm_flows_query_d = sketch_now.query_d_np(test_index_array)
                ...
                for drop_out in drop_out_list:
                    for set_epoch in epoch_list:
                        for set_batch in batch_list:
                            ...
                            torch.save(dnn1net.state_dict(), "cm_dnn_d_params_"+str(max_conf)+".pkl")