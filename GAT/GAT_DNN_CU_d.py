...
def conflict(index_np,dict_cu,w=16 * 10 ** 4,d=3,max_conf=3):
            ...
            for conf_flow_id in conf_flows_id_list:
                ...
                for flow_j in conf_flows_id_list:
                    if flow_j==conf_flow_id:
                        continue
                    # if (flow_j, conf_flow_id) not in edge_index_cu:
                    #     edge_index_cu[(flow_j, conf_flow_id)] = [0, 0, 0]
                    # edge_index_cu[(flow_j, conf_flow_id)][i] = 1
                ...
                while k<max_conf:
                    ...
                    # edge_index_cu[(flow_j, dummy_node)] = [0, 0, 0]
                    edge_index_cu.append([dummy_node,conf_flow_id])
                    k=k+1
    # # 将键从元组转换为列表
    # keys_list = [list(key) for key in edge_index_cu.keys()]
    # values_list = [value for value in edge_index_cu.values()]

    return torch.tensor(edge_index_cu, dtype=torch.float)



def load_cu_data_wide(w=16* 10 ** 4,flows_path="",file_now=179,real_sketch=False,max_conf=conf_num):
    ...
    dummy_node_features=np.zeros((max_conf*cu_d,cu_d))
    ...
    # test_cu = test_cu - 1
    # test_test_cu = test_cu[val_indices]
    # cu_y = test_test_cu - test_test_y

    #
    # print("CM acc=", np.mean(cu_y == 0))
    ...
    if label_type=="lg":
        ...
        test_cu[test_cu > flow_size_num] = flow_size_num
        node_labels = torch.tensor(test_y, dtype=torch.float)
    print("CU ARE=", np.mean(np.abs(test_cu / test_y - 1)))
    print("all 1 ARE=", np.mean((test_y - 1) / test_y))
    edge_index = conflict(index_np=test_index_array, dict_cu=dict_cu, w=cu_w, d=cu_d,max_conf=conf_num)
...
def train_gat_cora(config,file_path=""):
    ...
    node_features, node_labels, edge_index = load_cu_data_wide(w=w_globe,flows_path=file_path,max_conf=conf_num,real_sketch=True)
    ...
    torch.save(gat.state_dict(), "cu_gat_d_params_" + str(conf_num) + ".pkl")
...
if __name__ == '__main__':
    ...
    # model_path = 'cu_gat_d_params_3.pkl'
    # model_object = GAT(
    #     num_of_layers=config['num_of_layers'],
    #     num_heads_per_layer=config['num_heads_per_layer'],
    #     num_features_per_layer=config['num_features_per_layer'],
    ...
    #         print("alpha=", alpha_globe)
    #
    #         node_features, node_labels, edge_index = load_cu_data_wide(w=w_globe, flows_path=test_flows_path,
