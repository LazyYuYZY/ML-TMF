...
def conflict(index_np,dict_tower,w=16 * 10 ** 4,d=3,max_conf=3):
    ...
    return torch.tensor(edge_index_tower, dtype=torch.float)
...
def load_tower_data_wide(w=16* 10 ** 4,flows_path="",file_now=9,real_sketch=False,max_conf=conf_num):
    ...
    dummy_node_features=np.zeros((max_conf*tower_d,tower_d))
    ...
    edge_index = conflict(index_np=test_index_array, dict_tower=dict_tower, w=tower_w, d=tower_d,max_conf=conf_num)
...
def train_gat_cora(config,file_path=""):
    ...
    node_features, node_labels, edge_index = load_tower_data_zipf(w=w_globe,flows_path=file_path,alpha=alpha_globe,n=160000,max_conf=conf_num,zifp_sketch=False)
    ...
    torch.save(gat.state_dict(), "tower_gat_d_params_" + str(conf_num) + ".pkl")
...
if __name__ == '__main__':
    ...
    for w_globe in [16*10**4]:
        for alpha_globe in [2.0]:
            test_flows_path = "../zipf_testdata_set/testdata_flows/20_0200000_00.txt"
            print("w=", w_globe)
