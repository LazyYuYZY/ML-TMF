...
def conflict_gat(index_np,dict_tower,w=16 * 10 ** 4,d=3,max_conf=3):
    ...
    return torch.tensor(edge_index_tower, dtype=torch.float, device=device)

def conflict_dnn(index_np, dict_tower, w=22 * 10 ** 4, d=3, max_conf=2):
    a = np.array(dict_tower['a'])
    b = np.array(dict_tower['b'])
    p = np.array(dict_tower['p'])
    offset = dict_tower['offset']
...
def solution_real(w=1* 10 ** 4,flows_path="",file_now=0,real_sketch=False,dnn_conf_num=2,gat_conf_num=3):
    dnn_pkl="drop_tower_dnn_d_params_"+str(dnn_conf_num)+".pkl"
    gat_pkl="drop_tower_gat_d_params_"+str(gat_conf_num)+".pkl"
    ...
    new_dir="./testdata_set_"+str(T_slice)+"s/testdata_" + str(tower_w).zfill(6) + "_tower_"+str(T_slice)+"s"
    if not os.path.exists(new_dir):
        os.makedirs(new_dir)
    tower_test_path = new_dir +"/"+ str(file_now).zfill(5) + ".txt"
    # 若有对应tower数据集导入，没有就插入tower导出数据集
    if real_sketch:
        ...
        tower_sketch_now = tower_sketch(tower_d=tower_d, tower_w=tower_w, flag=1, dict_tower=dict_tower, tower_sketch_load=tower_sketch_load)
        tower_sketch_now.insert_dict(flows_data_onetime)
    ...
    test_x_ids = conflict_dnn(index_np=test_index_array, dict_tower=dict_tower,w=tower_w, d=tower_d,max_conf=dnn_conf_num)

    '''获取所在时刻冲突流的d个查询值'''
    test_x = np.full((test_x_ids.shape[0], tower_d * (1 + test_x_ids.shape[1])), 0)
    for i in range(tower_d * dnn_conf_num):
        conf_flows_id = np.where(test_x_ids[:, i] != -1)[0]
        test_x[conf_flows_id, i * tower_d:(i + 1) * tower_d] = tower_sketch_now.query_d_np(test_x_ids[conf_flows_id, i]).T
    test_x[:, tower_d * test_x_ids.shape[1]:] = tower_sketch_now.query_d_np(test_index_array).T
    ...
    dnn_model_object = DNN1(input_size=test_x.shape[1], deep_l=4 - 2)
    ...
    dummy_node_features = np.zeros((gat_conf_num * tower_d, tower_d))
    ...
    edge_index = conflict_gat(index_np=test_index_array,dict_tower=dict_tower,w=tower_w,d=tower_d,max_conf=gat_conf_num).t()
    ...
    # dnn_metrics = Metrics(real_val=test_y, pre_val=test_y)
    gat_metrics = Metrics(real_val=test_y, pre_val=test_gat[:-tower_d*gat_conf_num])
    dnn_metrics = Metrics(real_val=test_y,pre_val=test_dnn)
    tower_metrics = Metrics(real_val=test_y,pre_val=test_tower)

    gat_metrics.get_allval()
    # gat_metrics=0
    dnn_metrics.get_allval()
    tower_metrics.get_allval()

    return gat_metrics,dnn_metrics,tower_metrics

def solution_zipf(w=1* 10 ** 4,flows_path="",alpha=2.0,n=160000,zifp_sketch=False,numbering=0,dnn_conf_num=2,gat_conf_num=3):
    dnn_pkl="tower_dnn_d_params_"+str(dnn_conf_num)+".pkl"
    gat_pkl="tower_gat_d_params_"+str(gat_conf_num)+".pkl"
...
if __name__ == '__main__':
    ...
    w0=1*10**4
    # plt.ion()
    sketchs = ["GAT_Tower", "DNN_Tower", "Tower"]

    memory_list = [16]
    plots_np = np.full((3, len(memory_list), metrics_num, test_nums), 0,
                       dtype=float)  # 类型（sketch种类）k，横坐标i，纵坐标类型j,测试用例数量l
    for i in range(len(memory_list)):  # 变化内存
        ...
        for drop_flow in [0.01, 0.001, 0.0001, 0.00001, 0.000001]:
            ...
            for k in range(3):
                results_path = "./results/wide/" + "Bloom" + "/" + sketchs[k] + "_" + str(
                    int(-np.log10(drop_flow))) + ".txt"
...
#     memory_list = [16]#[10,14,20,26,64,112]
...
#             list_metrics=solution_zipf(w=160000,flows_path=test_flows_path,alpha=alpha_list[i],n=160000,zifp_sketch=True,numbering=l,dnn_conf_num=2,gat_conf_num=3)
#             for k in range(3):
#                 print(list_metrics[k].ARE_val)
#                 plots_np[k, i, 0, l]=list_metrics[k].ARE_val
#                 plots_np[k, i, 1, l] = list_metrics[k].AAE_val
